...
public class ParameterController {
	 ...
	 * required: 입력된 name 속성 값 파라미터 필수 여부 지정 ( 기본 값 true) 
	 * -> required=true인 파라미터가 존재하지 않는 다면 400( Bad Request)에러 발생
	 * ->""(빈문자열) 일때는 에러 발생 X ( 파라미터가 존재하지 않는것이 아니라
	 * name 속성값 ="" 로 넘어오기 때문에 )
	 * 
	 * 
	 * 
	...
	public String paramTest2(@RequestParam("title")String title,
							...
							//형변환이 필요한 경우에는 파라미터 value값이 필수 작성 되어있어야함!
	...
	public String paramTest3(@RequestParam("color")String[] colorArr,
		...
		log.debug("colrArr:" + Arrays.toString(colorArr));
	...
	public String paramTest4(/*@ModelAttribute*/ MemberDTO inputMember) {
		...
		//=="redirect:/param/main"
		//( 현재 위치가 중요! -> 현재 경로의 마지막 부분을 
		//redirect: 이주소로 변경하기 때문에 )
	}
