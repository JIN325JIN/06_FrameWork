...
UPDATE "MEMBER" SET
MEMBER_PW = '$2a$10$IPSafB3jt8hjaxf7EzIjROzZQeaycSuDRKOUl4d.76yhwc7hLzT3O'
WHERE MEMBER_NO=1;
...
VALUES(2,7992);--2번 회원이 7992번 게시글에 좋아요 누름
...
WHERE BOARD_CODE IN (4,5,6,7,8,9);
COMMIT;
...
--특정 게시판에 삭제 되지 않은 게시글 목록 조회
--단, 최신글이 제일 위에 존재하도록 조회
-- 작성일 : YYYY-MM-DD HH:24 가 아닌,
--몇초전, 몇분전, 몇시간전으로 표기하고싶다. 하루가 넘어가면 그땐 YYYY-DD-MM형식으로 조회
...
INSERT INTO "BOARD_IMG" VALUES(
	SEQ_IMG_NO.NEXTVAL, '/images/board/', '원본1.jpg', 'test1.jpg', 0, 3997
);

INSERT INTO "BOARD_IMG" VALUES(
	SEQ_IMG_NO.NEXTVAL, '/images/board/', '원본2.jpg', 'test2.jpg', 1, 3997
);

INSERT INTO "BOARD_IMG" VALUES(
	SEQ_IMG_NO.NEXTVAL, '/images/board/', '원본3.jpg', 'test3.jpg', 2, 3997
);

INSERT INTO "BOARD_IMG" VALUES(
	SEQ_IMG_NO.NEXTVAL, '/images/board/', '원본4.jpg', 'test4.jpg', 3, 3997
);

INSERT INTO "BOARD_IMG" VALUES(
	SEQ_IMG_NO.NEXTVAL, '/images/board/', '원본5.jpg', 'test5.jpg', 4, 3997
);
...
(SELECT COUNT(*) FROM "BOARD_LIKE" WHERE BOARD_NO = 3997) LIKE_COUNT,
(SELECT IMG_PATH || IMG_RENAME FROM "BOARD_IMG" WHERE BOARD_NO =3997 AND IMG_ORDER=0) THUMBNAIL,
(SELECT COUNT(*) FROM "BOARD_LIKE" WHERE BOARD_NO=3997 AND MEMBER_NO=2) LIKE_CHECK
FROM "BOARD"
JOIN "MEMBER" USING (MEMBER_NO)
WHERE BOARD.BOARD_DEL_FL ='N'
AND BOARD_NO = 3997
AND BOARD_CODE = 1;


--상세 조회하고 있는 게시글의 이미지 목록 조회 SQL
SELECT * FROM "BOARD_IMG" WHERE BOARD_NO = 3997 ORDER BY IMG_ORDER;




--상세 조회하고 있는 게시글의 댓글 목록 조회 SQL
-- LEVEK : ORACLE에서 제공하는 계층형 데이터를 조회할때 "레벨"에 있는지 알려주는 가상의 컬럼
SELECT LEVEL, C.* FROM
...
-------------------------------지울때
DELETE FROM "COMMENT"
WHERE COMMENT_NO IN (6035, 6036, 6037);
...
SELECT CHATTING_ROOM_NO
	...
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
		...
		R.PARTICIPANT,--NULL 이 아닌 경우 ( 로그인한 사람 오픈멤버일때는 참가자 타겟번호)
	...
	AND READ_FL = 'N' AND SENDER_NO != 1) NOT_READ_COUNT -- 내가 보내지않고, 안읽은것
	,(SELECT MAX(MESSAGE_NO) SEND_TIME 
	FROM MESSAGE M 
	WHERE R.CHATTING_ROOM_NO  = M.CHATTING_ROOM_NO) MAX_MESSAGE_NO -- 가장 최근메시지
