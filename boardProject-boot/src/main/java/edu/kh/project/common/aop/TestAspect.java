 ...
 * 
 * - 주요 비즈니스 로직 관련이 없는 부가적인 기능을 추가할 때 유용
 * 
 * ex) 코드 중간 중간에 로그 찍을 때, 트랜잭션 처리 하고 싶을 때 등...
 * 
 * 주요 어노테이션
 * - @Aspect : Aspect를 정의 하는데 사용되는 어노테이션으로, 클래스 상단에 작성함
 * - @Before(포인트컷) : 대상 메서드(포인트컷) 실행 전에 Advice를 실행함.
 * - @After (포인트컷) : 대상 메서드(포인트컷) 실행 후에 Advice를 실행함.
 * - @Around(포인트컷) : 대상 메서드(포인트컷) 실행 전/후로 Advice를 실행함.
 * 
 * */
...
public class TestAspect {
	...
	//<Pointcut 작성법>
	//execution([접근제한자] 리턴타입 클래스명 메서드명([파라미터(매개변수)]))
	//클래스명은 패키지명 부터 모두 작성!
