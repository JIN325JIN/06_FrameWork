...
/*
 * 스프링 예외 처리 방법 ( 우선 순위 별로 작성) 
 * 
 * 1. 메서드에서 직접 처리 (try - catch / throws)
 * 
 * 2.컨트롤러 클래스에서 클래스 단위로 모아서 처리 ( @ExceptionHandler 어노테이션을 지닌 메서드를 작성)
 * 
...
public class ExceptionController {
	...
	//@ExceptionHandler(예외 종류) : 어떤 예뢰를 다룰건지 작성
	...
	public String allExceptionHandler(Exception e,Model model) {
										//예외가 발생하면 매개변수 자리에 들어오는 e.printstacktrace찍어볼수 있음
										//model을 통해서 addAttribute하려고 model사용 (requeset scope)
 ...
 * 
 * 403 : 서버에서 외부의 접근을 거부 (Forbidden)
 * 
 * 404 : 요청 주소를 찾을 수 없음 (Not Found)
 * 
 * 405 : 허용되지 않는 메서드(요청 방식)(Method Not Allowed)
 * 
 * 500 : 서버 내부 오류 (Internal Server Error) (=>console창으로 확인하기)
 * */