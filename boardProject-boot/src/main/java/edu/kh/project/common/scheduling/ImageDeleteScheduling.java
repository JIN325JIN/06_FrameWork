...
public class ImageDeleteScheduling {
	...
	public void scheduling() {
		...
		//배열의 내용을 (깊은 복사)
		//System.arraycopy(복사할 배열, 몇번 인덱스 부터 복사할지, 
		//					새로운 배열, 새로운 배열의 몇번 인덱스부터 넣을지, 복사를 어디까지 할건지)
		...
		if(!serverImageList.isEmpty()){//서버에 이미지가 있을 경우
			for(File serverImage : serverImageList) {
				...
				//dbImageList 에 존재하지 않는 파일명 인지 확인
				
				//File.getName(): 서버 파일 이름 
		
				//List.indexOf(객체): List에 객체가 존재하면 , index 번호 반환
				//존재하지 않으면 -1 반환하는 메서드
...
* ex) 2025년 9월 16일 월요일 10시 30분 20초 cron="20 30 10 16 9 2 2025 " // 연도 생략 가능
*
*
*  @Scheduled(cron = "30 * * * * *") // 시계 초 단위가 30 인 경우 수행
*  @Scheduled(cron = "0,30 * * * * *")//시계 초 단위가 0,30인 경우 수행
*  @Scheduled(cron = "0 0 12 * * *")//정오마다 반복
*  @Scheduled(cron = "0 0 0 * * *")//자정마다 반복
*  @Scheduled(cron = "0 0 * * * *")//정시 반복
