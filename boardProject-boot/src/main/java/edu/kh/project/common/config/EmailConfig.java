...
public class EmailConfig {
	 ...
	 * 이메일 인증 절차 구현
	 * 
	 * 1. 관련된 테이블 생성 2. 구글 SMTP 세팅 ( 구글에서 비밀번호 발급...) 3.구글 SMTP를 나의 JAVA 어플리 케이션에서
	 * 이용할 수있도록 Config 세팅
	 * 
	 * 
	 ...
	 * 
	 * 
	 * 동작 방식 1. Spring boot 어플리 케이션에서 내가 이메일 보냄 2. Google SMTP 서버로 전송 GOOGLE 서버가 중간
	 * 배달자 역할 3. Google SMTP 서버가 이메일 주소에 맞는 "수신자"에게 이메일 서버를 통해( gmail.yaho,회사이메일서버)
	 * 를 통해 발송 4. 최종수신자 이메일을 받음
	 * 
	 * (참고) 기본 포트 25를 사용하지만, 보안 강화한 587이나 465가 자주 사용됨
	 */
	...
	public JavaMailSender javaMailSender() {
		/*
		 * Spring 에서 JavaMailSender를 구성하는 BEAN을 정의하기 위한 메서드.
		 * 
		 * JavaMailSender : 이메일을 보내는데 사용되는 인터페이스로 , JavaMailSenderImpl 클래스를 통해 구현됨.
		 * STMP서버를 사용하여 이메일을 보내기 위한 구성을 제공.
		 */
		...
		return mailSender;
		//위 처럼 각종 설정이 적용된 JavaMailSender를 Bean으로 등록하여 Spring애플리케이션에서 이메일을 보내기위한
		//구성을 제공함.
