...
public class FileConfig implements WebMvcConfigurer{
	...
	public void addResourceHandlers(ResourceHandlerRegistry registry) {
		...
		//Spring MVC에서 정적 리소스 ( CSS,JS ,이미지 파일) 등의 요청을 처리하기 위해 사용하는 클래스
		
		//URL 요청 패턴을 서버의 실제 파일 경로와 연결하여 클라이언트가 특정 경로로 정적파일에
		//접근 할 수 있도록 설정
		registry
		.addResourceHandler("/myPage/file/**")// 클라이언트의 요청주소 패턴
		.addResourceLocations("file:///C:/uploadFiles/test/");
		//요청을 연결해서 처리해줄 서버의 실제 폴더 경로
		
		
		//-> 클라이언트가 /myPage/file/** 패턴으로 이미지를 요청할 때
		//서버 폴더 경로 중 C;/uploadFiles/test/ 로 연결 하겠다 (여기서 이미지 찾겠다)	
	...
	public MultipartConfigElement configElement() {
		...
		//업로드 파일의 최대 크기, 메모리에서의 임시 저장 경로등 설정 가능
	...
	public MultipartResolver multipartResolver() {
		...
		// MultipartResolver는 클라이언트로 부터 받은  Multipart요청(폼태그의 enctype의 요청)을 처리하고,
		//이 중에서 업로드된 파일을 추출하여  MultipartFile객체로 제공 하는 역할
