...
/*Interceptor : 요청/ 응답 /뷰 완성후 가로채는 객체 (Spring 스펙 지원)
 * 
 ...
 * 
 * 
 * - afterCompletion ( 뷰 완성 ( forward코드 해석) 후) : View Resolver -> DispatcherServlet 사이 수행
 * */
@Slf4j
public class BoardTypeInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		//application scope : 
		//- 서버 시작시 부터 서버 종료시 까지 유지되는 Servlet 내장 객체
		//- 서버 내에 딱 한개만 존재!!
		//--> 모든 클라이언트가 공용으로 사용
		...
		return HandlerInterceptor.super.preHandle(request, response, handler); // true return 
	}
	
	//후처리 : 요청이 처리된 후 , 뷰가 렌더링 되기전에 실행되는 메서드
