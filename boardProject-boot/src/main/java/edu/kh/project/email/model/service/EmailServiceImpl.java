...
public class EmailServiceImpl implements EmailService {
	...
	private final JavaMailSender mailSender;//JavaMailSender : 실제 메일 발송을 담당하는 객체(EmailConfig설정이 적용된 객체)
	...
	public String sendEmail(String htmlName, String email) {
		...
		try {
			...
			//	 이메일의 본문, 이메일의 제목, 발송자 정보,수신자 정보 등을 포함
			
			//true :파일첨부를 사용할 것인지 여부 지정( 파일 첨부 및 내부 이미지 삽입 가능)
			...
			e.printStackTrace();
			return null; //메일 발송 실패시 null반환
		}
	}
	
	
	//HTML 템플릿에 데이터를 바인딩 하여 최종 HMTL 생성 메서드 
	...
	private boolean storeAuthKey(Map<String,String> map) {
		...
		//2.UPDATE 수행시 0 이 반환됨 == update안됨 ==insert된 내용이 없었다
		//insert 수행하면 된다
		if(result==0) {
	...
	//인증번호 발급 메서드
	//UUID를 사용하며 인증키 생성
	//UNIBERSALLY UNIQUE IDENTIFIER :전 세계 에서 고유한 식별자를 생성하기위한 표준
	//매우 낮은 확률로 중복되는 식별자를 생성
	
	//주로 데이터 베이스의 기본키, 고유한 식별자를 생성해야할 때 사용
