...
public class WebSocketConfig implements WebSocketConfigurer{
	...
	public void registerWebSocketHandlers(WebSocketHandlerRegistry registry) {
		...
		registry.addHandler(testWebSocketHandler, "/testSock") // 요청 주소
		// ws:// localhost/testSock으로 클라이언트가 요청을 하면 
		// testWebSocketHandler 처리하도록 등록
		.addInterceptors(handshakeInterceptor) // 클라이언트 연결 시 HttpSession을 가로채
											   // 핸들러에게 전달하는 handsahkeInterceptor 등록
