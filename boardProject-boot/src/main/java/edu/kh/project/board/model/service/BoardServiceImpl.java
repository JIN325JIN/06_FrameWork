...
public class BoardServiceImpl implements BoardService{
	...
	public Map<String, Object> selectBoardList(int boardCode, int cp) {
		 ...
		 * : 지정된 크기만큼 건너 뛰고(offset) 제한된 크기 만큼(limit)의 행을 조회하는 객체 
		 * 
		 * 
		 * */
		...
		//Mapper 메서드 호출시 원래 전달 할 수있는 매개변수 1개
		//->2개를 전달 할 수있는 경우가 있음
		//rowBounds를 이용할 때 !
		//-> 첫번재 매개변수 -> SQL에 전달할 파라미터
		//-> 두번째 매개변수 -> RowBounds객체 전달 
		List<Board> boardList = mapper.selectBoardList(boardCode,rowBounds);
		
		
		//log.debug("boardList결과 : {}",boardList);
		
		//4.목록 조회 결과 + Pgination 객체를 Map으로 묶어서 반환
	...
	public int boardLike(Map<String, Integer> map) {
		...
		//1. 좋아요가 체크된 상태의 경우 (likeCheck==1)
		//-> BOARD_LIKE 테이블에 DELETE 수행
		if(map.get("likeCheck")==1) {
			result = mapper.deleteBoardLike(map);
		}else {
			//2. 좋아요가 해제된 상태의 경우 (likeCheck==0)
