...
public class EditBoardServiceImpl implements EditBoardService {
	...
	public int boardInsert(Board inputBoard, List<MultipartFile> images) throws Exception {
		...
		//result = INSERT 결과 ( 삽입 성공한 행의 갯수 0or 1)
		...
		//boardNo 가 inputBoard에 저장된 상태!(얕은 복사 개념)
		...
		//images 리스트에서 하나씩 꺼내어 파일이 실제로 있는지 없는지 검사 
		for(int i =0;i<images.size();i++) {
			...
			if(!images.get(i).isEmpty()) {
				...
				BoardImg img = BoardImg.builder().imgOriginalName(originalName)
						...
						.imgOrder(i).uploadFile(images.get(i))
						.build();
	...
	public int boardUpdate(Board inputBoard,List<MultipartFile> images,
		...
		//2. 기존0 -> 삭제된 이미지 ( deleteOrderList)가 있는 경우
		...
		for (int i =0;i<images.size();i++) {
			...
			if(!images.get(i).isEmpty()) {
				...
				//모든 값을 저장할 DTO생성 (BoardImg)
				BoardImg img = BoardImg.builder().imgOriginalName(originalName)
						.imgRename(rename).imgPath(webPath).boardNo(inputBoard.getBoardNo())
				...
				//4. 업로드 하려는 이미지 정보를 이용해서 수정 or 삽입 수행
