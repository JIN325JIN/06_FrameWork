...
if(updateInfo != null) {
    ...
    updateInfo.addEventListener("submit", async e => {
        ...
        //위의 모든 검증을 통과 했을 때만 폼 제출
        updateInfo.submit();

        /*
        submit 이벤트 핸들러 함수 내에서 fetch를 이용한 비동기 함수를 실행중 일때 
        e.preventDefault()가 비동기 함수 이후에 호출되면 이미 form이 submit되어버린
        다음일 수 있음
        -> 비동기 작업중에 form의 기본 제출 동작 일어나는 가능성이 생김!
        */
    });
}
...
if(profileForm!=null){
    ...
    //절대 경로 방식으로 기본 이미지 URL설정
    const defaultImageUrl = `${window.location.origin}/images/user.png`;
    //윈도우가 자바스크립트 내장객체 : location 한테 origin꺼내와서 쓰면 오리진 주소가 나옴
    //http://localhost/ (포트번호 포함)+ images/user.png
    //정적 리소르를 받아올수 있는 경로를 저장해놓음

    let statusCheck = -1; // -1 : 초기상태 ,0: 이미지 삭제, 1: 새 이미지 선택했다는 의미
    // 현재 상태 체크용 : 이미지가 변했는지, 삭제되었는지, 새 이미지가 선택되었는지 기억해주는 상태값
   
    let previousImage = profileImg.src; //이전 이미지 URL기록 (초기 상태의 이미지 저장)
    //IMG태그에 작성하는 값 src = 미리보기 이미지를 띄울 URL주소
   
    let previousFile = null; //이전에 선택된 파일 객체를 저장
    //input (type = file) 태그가 작성한 값 = 서버에 실제로 제출되는 File 객체
    ...
    imageInput.addEventListener("change",()=>{
        ...
        const file = imageInput.files[0];// 실제로 선택한 File객체 가져오기

        console.log(file);//fileList  객체


        if(file){ //파일이 선택이 된 경우


            if(file.size <=MAX_SIZE){ //현재 선택한 파일의 크기가 허용 범위 이내인 경우(정상인경우) 

                const newImageUrl = URL.createObjectURL(file);//임시 URL생성 
                //URL.createObjectURL(파일):웹에서 접근가능한 임시 URL 반환
                console.log(newImageUrl);

                profileImg.src = newImageUrl;//미리보기 이미지 설정 (img태그의 src에 선택한 파일 임시경로 대입)
            ...
            //이전 미리보기 이미지로 복원(img태그)
            profileImg.src= previousImage;

            //이전 선택한 파일로 복원(input태그)
            if(previousFile){ //이전 파일이 존재한다면
                const dataTransfer = new DataTransfer();
                //-> DataTransfer : 자바 스크립트 파일을 조작할 때 사용되는 인터페이스.
                //DataTransfer가 제공하는 메서드 중 
                ...
                //->input태그의 files속성은 FileList만 저장 가능한 형태이기 때문에
                //DataTransfer를 이용하여 현재 File 객체를 FileList변환하여 할당 진행
                dataTransfer.items.add(previousFile);
                imageInput.files=dataTransfer.files;//  FileList반환
    ...
    deleteImage.addEventListener("click",()=>{
        ...
        imageInput.value="";//input태그 파일값 초기화
        ...
        previousFile=null; //이전 파일 기록하는 변수에 null로 초기화(x버튼 눌렀으니 비우기)

        }else{//기본 이미지 상태에서 삭제 버튼 클릭시 상태를 변경하지 않음
