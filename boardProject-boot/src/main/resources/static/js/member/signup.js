...
memberEmail.addEventListener("input",(e)=>{
  ...
  //이메일 형식으로 작성
  //@와 도메인부분
  //도메인 부분은 최소 2개의 문자로 끝나야함 (.com , .net, .kr,.org)
  ...
  if(!regExp.test(inputEmail)){
    ...
    emailMessage.classList.remove('confirm');//초록색 글씨 confirm 제거
    checkObj.memberEmail =false; //유효하지 않음 이메일임을 기록
...
sendAuthKeyBtn.addEventListener("click",()=>{
  ...
  //새로운 인증번호 발급을 원하는것 이기 때문에
  //새로 발급 받은 인증번호 확인전까지는  checkObj.authKey는 false
  checkObj.authKey=false;
  //인증번호 발급 관련된 메시지 지우기
  authKeyMessage.innerText="";
...
//매개변수 전달받은 숫자가 10미만인 경우 (한자리) 앞에 0을 붙여서 반환
...
//--------------4/22여기 까지함
//인증하기 버튼 클릭시
//입력된 인증번호를 비동기로 서버로 전달
//비동기로 작성 : 인증하기 버튼을 눌렀을때 동기식 하면 전체 화면 새로고침
...
checkAuthKeyBtn.addEventListener("click",()=>{
  ...
  if(authKey.value.length<6||authKey.value.length>=7){//인증번호가 제대로 입력 안된 경우 (길이가 6미만,7이상)
...
fetch("/email/checkAuthKey",{
  method:"POST",
  headers:{"Content-Type":"application/json"},
  body : JSON.stringify(obj)//obj라는 JS객체를 JSON으로 변경
})
.then(resp=>resp.text())
.then(result=>{
  //1 or 0
...
memberPw.addEventListener("input",e=>{
  ...
  if(inputPw.trim().length===0){
    ...
    checkObj.memberPw=false; //비밀번호가 유효하지않다고 표시
  ...
  pwMessage.classList.add("confirm");
  pwMessage.classList.remove("error");
  checkObj.memberPw=true;//유효한 비밀번호임을 암시
...
//2)정규식 검사
const regExp =/^[가-힣\w\d]{2,10}$/;//한글 : 가-힣 영어:\word 숫자:\d 2에서10{2,10}
...
memberTel.addEventListener("input",e=>{
  ...
  if(inputTel.trim().length===0){
    ...
    telMessage.classList.remove("error","confrim");
  ...
  if(!regExp.test(inputTel)){
    telMessage.innerText="유효하지않은 전화번호 형식 입니다!";
...
const signupForm =document.querySelector("#signUpForm");//form태그
...
signupForm.addEventListener("submit",e=>{
  ...
  for(let key in checkObj){ //checkObj 요소의 key값을 순서대로 꺼내옴
    ...
    if(!checkObj[key]){//현재 접근 중인 checkObj[key]의 value값이 false인 경우

      let str ;// 출력할 메시지를 저장할 변수

      switch(key){
        case "memberEmail" : str = "이메일이 유효하지 않습니다.";
        break;
        case "authKey" : str = "이메일이 인증되지 않았습니다.";
        break;
        case "memberPw" : str ="비밀번호가 유효하지 않습니다.";
        break;
        case "memberPwConfirm" : str="비밀번호가 일치하지 않습니다.";
        break;
        case "memberNickname" : str ="닉네임이 유효하지 않습니다.";
        break;
        case "memberTel" : str ="전화번호가 유효하지 않습니다.";
        break;
      }
      alert(str);
      document.getElementById(key).focus();//해당 input초점 이동
