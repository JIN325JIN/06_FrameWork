//REST(RE: REpresentational State Transfer) API

// /comments(GET)
// /comments(POST)

// - 자원( 데이터,파일)을 이름(주소)으로 구분하여 자원의 상태 (state)를 주고 받는 것

//-> 자원의 이름 (주소)를 명시하고 HTTP Method (GET,POST,PUT,DELETE)를 이용해 지정된 자원에 대한 
//CRUD진행
...
addComment.addEventListener("click",()=>{
  ...
  if(loginMemberNo==null){
    //boardDetail.html에서 얻어옴 session scope내부에 있음
    alert("로그인 후 이용해주세요.");
    return;
  }
  ...
  .then(result=>{
    ...
    if(result>0){
      ...
      commentContent.value="";//작성한 댓글 내용 지우기 ( 비동기 요청이라 화면 깜빡 X)
    }else{
      alert("댓글 등록 실패..");
    }
...
const showInsertComment = (parentCommentNo, btn) => {
  ...
  //매개변수에 + 문자열 + 작성시 Number 타입으로 형변환 하라
