...
/*
fetch()API
비동기 요청을 수행하는 최신 JAVASCRIPT API중 하나

- Promise는 비동기 기반
promist는 비동기 작업의 결과를 처리하는 방법 중 하나
-> 어떤 결과가 올지는 모르지만 반드시 결과를 보내주겠다는 약속.
-> 비동기 작업이 맞이할 완료 또는 실패와 그 결과 값을 나타냄.

-> 비동기 작업이 완료 되었을때 실행할 콜백함수를 지정하고,
해당 작업의 성공 또는 실패 여부를 처리 할 수 있도록 함.

Promist객체는 총 3가지 상태를 가짐
-Pending (대기중) : 비동기 작업이 완료 되지 않은 상태
-Fullfilled(이행됨) : 비동기 작업이 성공적으로 완료+된 상태
...
function getTotalCount() {//함수 정의
  ...
  fetch("/ajax/totalCount")//서버로 "/ajax/totalCount"로 GET 요청
    ...
    .then(result => { //첫번째 콜백함수가 완료된 후 호출되는 콜백함수
      ...
      //#totalCount인 span태그의 내용으로 result 값을 대입
...
function getCompleteCount() {
  ...
  fetch("/ajax/completeCount")
    ...
    .then(result => {
      ...
      //id가 completeCount인 요소 ( #completeCount)에 내용으로  result값 출력
...
addBtn.addEventListener("click", () => {
  ...
  //-전달 데이터 : todoTitle값, todoConetent값
  //JS <-> java
  ...
  //todoTitle과 todoContent를 저장한 JS객체
  const param = {
    //key:value
  ...
  fetch("/ajax/add", {
    ...
    .then(resp => resp.text())//반환 값을 text로 변환
    .then(result => {
      ...
      if (result > 0) { //성공
        ...
        //할일이 새롭게 추가되었으므로
        //-> 전체 Todo개수 조회하는 함수 재호출
        getTotalCount();

        //-> 전체 Todo목록 조회하는 함수 재호출 예정
        selectTodoList();
...
const selectTodoList = () => {
  ...
  fetch("/ajax/selectList")
    ...
    .then(todoList => {
      ...
      //JSON.parse(JSON데이터): string->JS object
      ...
      for (let todo of todoList) {//향상된 for문 
        ...
        //JS객체에 존재하는 key모음 배열 생성
        const arr = ['todoNo', 'todoTitle', 'complete', 'regDate'];
        //todoContent는 일부러 안가져옴 -> table에 표현 안할거라서 안 얻어옴!!
        ...
        for (let key of arr) {
          ...
          if (key === 'todoTitle') {
            ...
            //a태그 클릭 시 페이지 이동 막기 (비동기 요청으로 돌리기 )
            a.addEventListener("click", e => {
              e.preventDefault();//기본 이벤트 방지



              //할일 상세 조회 비동기 요청 함수 호출
              selectTodo(e.target.href);
            });

            continue;//break하면 title이후가 수행안되니깐 continue로 해야함 다음반복으로 넘어가게
...
deleteBtn.addEventListener("click", () => {
  ...
  //취소 클릭시 아무일도 일어나지 X (해당 이벤트 함수 종료)
  ...
  //  삭제 비동기 요청(1.get / 2. post /3.delete중에 DELETE방식!)
  fetch("/ajax/delete", {
    method: "DELETE",//delete방식 요청 -> @DELETEMAPPING 처리
    headers: { "Content-Type": "application/json" },
    body: todoNo//todoNo 단일 값 하나는 JSON형태로 자동 변환되어 전달 됨
...
changeComplete.addEventListener("click", () => {
  ...
  //{"todoNo":2,"complete","Y"}이런식으로 넘어갈것

  //비동기로 완료 여부 변경 요청 ( PUT요청 방식)
...
updateView.addEventListener("click", () => {
  ...
  //HTML화면에서 줄 바꿈이 <br>로 인식되고 있는데
  //textarea에서는 /n으로 바꿔줘야 실제 줄바꿈으로 인식되어 출력된다!




  //수정 레이어 -> 수정 버튼에 datd-todoNo-no속성 추가
